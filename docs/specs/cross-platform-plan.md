...
10. ✅ **Path handling on Windows: a three-way rule, not a blanket one.** The affected sites are `main.rs:106` and
    ...
    - **Finished in M5:** `reveal_path_chain`'s ancestor walk is on the policy too, through `PathPolicy::ancestors` and
      `PathPolicy::component_count`, so its Windows shapes (drive roots, `\\?\` spellings, NAS shares, NTFS casing) are
      all asserted from a Mac.
