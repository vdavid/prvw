...
| `dcp/` (Phase 3.2 + 3.3) | Adobe Digital Camera Profile support — per-camera color refinement. Submodules: `parser.rs` reads the `.dcp` TIFF-like binary, `embedded.rs` (Phase 3.3) reads the same profile tags straight from a DNG's main IFD, `apply.rs` applies `ProfileHueSatMap` as a trilinearly-interpolated 3D LUT in HSV (cyclic hue, clamped sat/val), `discovery.rs` searches `$PRVW_DCP_DIR` + Adobe Camera Raw's install dir and matches by `UniqueCameraModel`. Two discovery paths feed the same applier: embedded (wins, DNG carries its own profile — Pixel, Galaxy, iPhone ProRAW) and filesystem (fallback, user drops a `.dcp` into `$PRVW_DCP_DIR`). Runs post-highlight-recovery, pre-tone-curve. Silent no-op when neither path yields a match. Deferred: `LookTable`, `ProfileToneCurve`, dual-illuminant interpolation, `ForwardMatrix` swap (see `docs/notes/raw-support-phase3.md`) |
