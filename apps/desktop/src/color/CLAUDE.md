...
| `dcp/` (Phase 3.2 + 3.3 + 3.4) | Adobe Digital Camera Profile support — per-camera color refinement. Submodules: `parser.rs` reads the `.dcp` TIFF-like binary, `embedded.rs` reads the same profile tags straight from a DNG's main IFD, `apply.rs` applies `ProfileHueSatMap` as a trilinearly-interpolated 3D LUT in HSV (cyclic hue, clamped sat/val), `discovery.rs` searches `$PRVW_DCP_DIR` + Adobe Camera Raw's install dir and matches by `UniqueCameraModel`, `illuminant.rs` (3.4) estimates scene color temperature from white-balance coefficients and blends dual-illuminant `HueSatMap1`/`HueSatMap2`. Two discovery paths feed the same applier: embedded (wins, DNG carries its own profile — Pixel, Galaxy, iPhone ProRAW) and filesystem (fallback, user drops a `.dcp` into `$PRVW_DCP_DIR`). Runs post-highlight-recovery, pre-tone-curve. HueSatMap → LookTable → (optional tone-curve swap) order. Silent no-op when neither path yields a match. Still deferred: `ForwardMatrix` swap, full iterative CCT convergence (see `docs/notes/raw-support-phase3.md`) |
