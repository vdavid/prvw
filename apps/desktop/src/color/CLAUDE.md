...
| `clarity.rs`         | Local-contrast ("clarity") enhancement for RAW output (Phase 6.2). Same separable-Gaussian unsharp-mask algorithm as `sharpen.rs`, just at a much larger radius (`DEFAULT_RADIUS = 10 px`, vs. 0.8 px for capture sharpening). Lifts midtone features — shape silhouettes, textures — that survive display downscaling, so the image reads crisper at every zoom level. Luminance-only, same invariants. Thin wrapper: `apply_clarity_rgba8_inplace_with` / `apply_clarity_rgba16f_inplace_with` delegate to `sharpen::sharpen_*_with` with clarity's default constants. Runs post-ICC, before capture sharpening (midtone lift → fine edges). Users tune radius (px) and amount (0–1) via Settings → RAW → Detail |
