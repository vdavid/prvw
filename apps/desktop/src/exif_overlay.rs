...
//! - `overlay::build` produces the visual layers: a backdrop pill plus one
//!   text block per row. Hidden entirely when the current image has no
//!   EXIF, even if the user toggled the panel on.
