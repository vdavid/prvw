...
/// matching `navigation::preloader::MAX_PRELOAD_AHEAD`. Only a priority
/// boundary: previews inside it are generated first because a full decode may
/// beat them there. Kept as its own constant rather than reading
/// `preloader::preload_count()`, because getting the order wrong costs an
/// ordering slot rather than memory, and the coupling would buy nothing.
