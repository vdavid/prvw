...
<<<<<<< HEAD
UI, the settings window, onboarding, display-profile matching, QuickLook previews, and the updater are all macOS-only.
Windows has a native menu bar with working accelerators, its own About box under Help, and the cross-platform core
(decode, RAW pipeline, color transform, settings, navigation, and the header-only dimension read that sizes the window
before the first pixel paints); Linux has the core and no menu bar at all. Windows deliberately gets no onboarding
